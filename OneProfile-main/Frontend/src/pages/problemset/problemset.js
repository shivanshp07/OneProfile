...
function ProblemSet() {
    ...
    if (GUserName) {
        ...
        GUserName = ''; // Handle case when the user is not logged in
    }

    async function checkFun(link) {
        let username = localStorage.getItem("username");
        if (!username) return false; // Handle case when the user is not logged in
        let rem = await fetch(""); // Provide appropriate URL and logic here
        return true;
    }
    ...
    async function checkBoxClick(link) {
        ...
        if (!userName) return; // Early return if the user is not logged in
            ...
            >
                ...
                <FormControl style={{ margin: '12px' }}>
                    <Select
                        ...
                        variant='standard'
                    >
                        <MenuItem value="">Sheet Name</MenuItem>
