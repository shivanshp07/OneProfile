...
// const columns = [
//   { id: 'site', label: 'Site', minWidth: '300' },
//   { id: 'name', label: 'Name', minWidth: '400', align: 'right' },
//   { id: 'start_time', label: 'Start', align: 'right', sort: true },
//   { id: 'end_time', label: 'End', align: 'right', sort: true },
// ];
...
//         'https://clist.by/api/v2/contest/?username=YOUR_USERNAME&api_key=YOUR_API_KEY',
//         { headers: { 'Content-Type': 'application/json' } }
//       );
//       const data = await result.json();
//       const formattedData = data.objects.map((contest) => ({
//         site: contest.resource.name,
//         name: contest.event,
...
//   const filteredRows = rows.filter((row) => {
//     const siteName =
//       row.site.toLowerCase() +
//       row.name.toLowerCase() +
//       row.start_time.toLowerCase() +
//       row.end_time.toLowerCase();
//     if (platForm === 'All Platform')
//       return siteName.includes(searchText.toLowerCase());
//     else
//       return (
//         siteName.includes(platForm.toLowerCase()) &&
...
//         <h1 style={{ marginLeft: '15px', color: '#0048ffe6' }}>Upcoming Contests</h1>
//         <Box>
//           <Select
//             id="platform-select"
//             value={platForm}
...
function Ratings() {
  ...
  async function showRatings() {
    try {
      setLoading(true);
      const result = await fetch('https://clist.by/api/v2/contest/', {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'ApiKey ayush_kumar__:eefb537efe8b1822b1d827df4c23ed4b641f0609'
        }
      });

      if (!result.ok) {
        throw new Error(`Error ${result.status}: Unauthorized or invalid API key`);
      }

      const data = await result.json();
      const formattedData = data.objects.map((contest, index) => ({
        id: index, // Use a unique identifier for key
        site: contest.resource.name,
        name: contest.event,
        start_time: contest.start,
        end_time: contest.end,
              ...
              {!loading &&
                filteredRows
                  ...
                  .map((row) => (
                    <TableRow
                      hover
                      key={row.id} // Use a unique key here
                      onClick={() => window.location.href = row.url}
                      style={{ cursor: 'pointer' }}
                    >
                      {columns.map((column) => (
